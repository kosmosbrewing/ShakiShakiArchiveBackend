...
const discountPercent = computed(() => {
  ...
  return Math.round(
    ((originalPrice.value - price.value) / originalPrice.value) * 100
  );
});

const isOutOfStock = computed(
  () => !props.product.isAvailable || (props.product.stockQuantity ?? 0) <= 0
);
...
<template>
  ...
  >
    <!-- 이미지 -->
    <div
      class="relative mb-4 aspect-square overflow-hidden rounded-lg bg-muted"
    >
      <img
