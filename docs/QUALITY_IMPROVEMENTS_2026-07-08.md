...
> **역사 스냅샷:** 이 문서는 2026-07-08 당시 코드 점검, 운영 전제, 우선순위를 보존합니다. 현재 상태 판단에는 [MEMORY](../MEMORY.md), [Architecture](./ARCHITECTURE.md), 실제 코드를 사용하세요. 아래 line number와 "운영 사용" 주장은 현재 환경에서 다시 검증해야 합니다.

4개 관점(보안 / 안정성·가용성 / DB·성능 / 결제·주문 정합성) 병렬 코드 점검 결과.
프론트엔드 품질 개선(프론트 repo `docs/QUALITY_IMPROVEMENTS_2026-07-08.md`)과 같은 방식으로 수행.
심각도·우선순위 순으로 정리. **기존 기능·가용성을 훼손하지 않는 선에서 개선하는 것이 전제.**

**진행 현황 (2026-07-08 구현)**: 착수 순서 1~3(A-8, A-1, A-5/A-6) 완료.
- **A-8 ✅**: 토스/네이버페이(결제형·주문형) 3개 라우터에 router.use 일괄 게이트.
  **구현 중 발견**: `/payments/naverpay`·`/payments/kakaopay`가 `/payments`(토스) 뒤에
  마운트되어 있어 라우터 게이트가 카카오페이까지 차단하는 문제 → routes/index.ts에서
  구체 경로 우선으로 마운트 재정렬. 스모크 테스트로 토스/네이버 503 + 카카오 정상 확인.
- **A-1 ✅**: updateCartItem/deleteCartItem에 userId 필터, 미소유 접근은 404.
- **A-5/A-6 ✅**: 시그널 핸들러를 db.ts→index.ts 이관, httpServer.close(drain) →
  closePool → exit 순서 + 10초 타임아웃. unhandledRejection/uncaughtException은
  로깅 + 텔레그램 알림(uncaughtException은 안전 종료 후 재기동 유도).
  SIGTERM 스모크 테스트로 종료 순서 확인.
- **A-2 ✅**: `updateOrderPayment`에 `expectedStatuses` 원자적 가드 옵션 추가,
  카카오 선점 경로에 적용(가드 0건 → 부작용 스킵 + already-paid success 리다이렉트).
  추가 보완: 동시 콜백으로 카카오 approve가 에러난 경우에도 주문이 이미 확정됐으면
  1.5초 재확인 후 success로 안내 (성공 결제가 실패 화면으로 보이는 것 방지).
- **A-3 ✅ (하이브리드 — 운영 DB 무변경으로 방식 변경)**: 컬럼 추가 대신
  인메모리 Map 유지 + `req.session.kakaopayTids`(PG 세션 스토어) 백업.
  근거: 콜백은 의도적으로 세션 비의존 설계(쿠키 유실 환경 대응)라 순수 세션 방식은
  회귀 위험 → Map 우선 + 세션 폴백이면 어떤 시나리오도 현행보다 나빠지지 않으면서
  배포/재시작 시 결제 실패를 구제. tid 조회 hit 소스("memory"/"session")와
  both-miss를 로그로 관측 — both-miss가 실제 발생하면 그때 컬럼 방식 검토.
- 다음: 착수 순서 5(A-4/B-1/B-2 환불 경로) → 6(A-7 문의 페이지네이션).
