...
router.get("/:id", async (req, res) => {
  try {
    ...
    const message = error instanceof Error ? error.message : "문의 조회 실패";
    res.status(500).json({ message });
  }
});
...
// 내 문의 목록 조회
router.get("/my/list", isAuthenticated, async (req, res) => {
  try {
    const userId = req.session.userId!;
    ...
    const message = error instanceof Error ? error.message : "내 문의 목록 조회 실패";
    res.status(500).json({ message });
  }
});
...
router.post("/", isAuthenticated, async (req, res) => {
  try {
    ...
    const message = error instanceof Error ? error.message : "문의 등록 실패";
    res.status(500).json({ message });
  }
});
...
// 답변 등록 (관리자만)
router.post("/:id/replies", isAdmin, async (req, res) => {
  try {
    const userId = req.session.userId!;
    ...
    const message = error instanceof Error ? error.message : "답변 등록 실패";
    res.status(500).json({ message });
  }
});
...
// 답변 삭제 (관리자만)
router.delete("/replies/:replyId", isAdmin, async (req, res) => {
  try {
    ...
    const message = error instanceof Error ? error.message : "답변 삭제 실패";
    res.status(500).json({ message });
  }
});
